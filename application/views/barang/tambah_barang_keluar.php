<?php $this->load->view('template/header');?>
<?php $this->load->view('template/sidebar');?>
<!-- Main Content -->
<div class="main-content">
  <section class="section">
    <div class="section-header">
      <h1><?= $title?></h1>
      <div class="section-header-breadcrumb">
        <div class="breadcrumb-item active"><a href="#">Data Penjualan</a></div>
        <div class="breadcrumb-item">Tambah Penjualan</div>
      </div>
    </div>

    <div class="section-body">
      <div class="row">
        <div class="col-12">
          <div class="card">
            <form action="<?= base_url('tambah-barang-keluar'); ?>" method="post">
              <div class="card-header">
                <h4>Form Tambah Penjualan</h4>
              </div>
              <div class="card-body">
                <div class="row">
                  <div class="col-lg-4 form-group">
                    <div class="form-group">
                      <label>Transaksi</label>
                      <input type="text" name="id_penjualan" class="form-control" value="<?= set_value('id_penjualan', $id_penjualan); ?>" readonly="">
                      <?= form_error('id_penjualan', '<span class="text-danger small">', '</span>'); ?>
                    </div>
                  </div>
                  <div class="col-lg-4 form-group">
                    <div class="form-group">
                      <label>Tanggal</label>
                      <input type="date" name="tanggal" class="form-control" value="<?= set_value('tanggal', date('Y-m-d')); ?>" required="">
                      <?= form_error('tanggal', '<span class="text-danger small">', '</span>'); ?>
                    </div>
                  </div>
                  <div class="col-lg-4 form-group">
                    <div class="form-group">
                      <label>Keterangan</label>
                      <select name="mode_jual" class="form-control">
                        <option selected="" value="Harga Jual Toko">Harga Jual Toko</option>
                        <option value="Harga Reseller Cash">Harga Reseller Cash</option>
                        <option value="Harga Reseller Tempo">Harga Reseller Tempo</option>
                        <option value="Harga Go Food">Harga Go Food</option>
                        <option value="Harga Grab Food">Harga Grab Food</option>
                        <option value="Harga Shopee Food">Harga Shopee Food</option>
                      </select>
                      <?= form_error('mode_jual', '<span class="text-danger small">', '</span>'); ?>
                    </div>
                  </div>
                </div> 
                <div class="row">
                  <div class="col-lg-6">
                    <a style="color: white;" class="btn btn-info" onclick="tambahItem()"><i class="fa fa-plus"></i> Tambah Item</a>
                    <a style="color: white;" class="btn btn-danger" onclick="hapusItem()"><i class="fa fa-trash"></i> Reset Item</a>
                  </div>
                </div>
                <div class="row">
                  <div class="col-lg-6 form-group">
                    <label>Pilih Item</label>
                    <select class="form-control" name="id_barang[]" id="" data-live-search="true">
                      <option disabled selected>-- Pilih Item --</option>
                      <?php foreach ($barang as $b):?>
                      <option value="<?= $b['id_barang']?>" <?= set_value('id_barang') == $b['id_barang'] ? 'selected' : '' ; ?> ><?= $b['nama_bolu'].' || Stok : '.$b['stok'] ?></option>
                      <?php endforeach;?>
                    </select>
                    <?= form_error('id_barang', '<span class="text-danger small">', '</span>'); ?>
                  </div>
                  <div class="col-lg-3 form-group">
                    <label>Jumlah</label>
                    <input type="number" name="jumlah[]" class="form-control" value="<?= set_value('jumlah'); ?>" required="">
                    <?= form_error('jumlah', '<span class="text-danger small">', '</span>'); ?>
                  </div>
                  <div class="col-lg-3 form-group">
                    <label>Exp</label>
                    <input type="date" name="exp[]" class="form-control" value="<?= set_value('exp'); ?>" required="">
                    <?= form_error('exp', '<span class="text-danger small">', '</span>'); ?>
                  </div>
                </div> 
                <div class="row form-item">
                  
                </div> 
              </div>

              <div class="card-footer text-right">
                <a href="<?= base_url('barang-keluar');?>" class="btn btn-light"><i class="fa fa-arrow-left"></i> Kembali</a>
                <button type="reset" class="btn btn-danger"><i class="fa fa-sync"></i> Reset</button>
                <button type="submit" class="btn btn-primary"><i class="fa fa-save"></i> Simpan</button>
              </div>
            </form>
          </div>
        </div>
      </div>
    </div>
  </section>
</div>
<script type="text/javascript">
    function tambahItem(){
    $(".form-item").append('<div class="col-lg-6 form-group"><select class="form-control" name="id_barang[]" id="" data-live-search="true"><option disabled selected>-- Pilih Item --</option><?php foreach ($barang as $b):?><option value="<?= $b['id_barang']?>" <?= set_value('id_barang') == $b['id_barang'] ? 'selected' : '' ; ?> ><?= $b['nama_bolu'].' || Stok : '.$b['stok'] ?></option><?php endforeach;?></select><?= form_error('id_barang', '<span class="text-danger small">', '</span>'); ?></div><div class="col-lg-3 form-group"><input type="number" name="jumlah[]" class="form-control" value="<?= set_value('jumlah'); ?>" required=""><?= form_error('jumlah', '<span class="text-danger small">', '</span>'); ?></div><div class="col-lg-3 form-group"><input type="date" name="exp[]" class="form-control" value="<?= set_value('exp'); ?>" required=""><?= form_error('exp', '<span class="text-danger small">', '</span>'); ?></div>').children(':last');
    
    }

    function hapusItem() {
        $(".form-item").empty().lastChild;


    }
</script>
<?php $this->load->view('template/footer');?>