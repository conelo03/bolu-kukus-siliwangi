...
class M_barang extends CI_Model {
	...
	{
		$this->db->select('*, substr(id_penjualan, 10) as id');
		$this->db->from('penjualan');
		$this->db->order_by('tanggal', 'DESC');
		$this->db->order_by('cast(id as unsigned)', 'DESC');
		//$this->db->order_by('LENGTH(id_penjualan), id_penjualan', 'DESC');
        return $this->db->get();
	}
	...
	{
		$this->db->select('*,penjualan.keterangan as keterangan, penjualan_detail.harga_jual, penjualan_detail.harga_beli, substr(penjualan.id_penjualan, 10) as id');
		$this->db->from('penjualan');
