...
class Barang extends CI_Controller {
	...
	public function tambah()
	{
		...
		if (!$this->form_validation->run()) {
			...
			$data_user	= [
				...
				'harga_jual_go_food'		=> $data['harga_jual_go_food'],
				'harga_jual_grab_food'		=> $data['harga_jual_grab_food'],
				'harga_jual_shopee_food'		=> $data['harga_jual_shopee_food'],
				'diskon'			=> $data['diskon'],
				'ppn'				=> $data['ppn'],
	...
	public function tambah_barang_keluar()
	{
		...
		if (!$this->form_validation->run()) {
			...
			->from('penjualan')
			->where("DATE_FORMAT(tanggal, '%Y-%m-%d') =", date('Y-m-d'))
			->order_by('LENGTH(id_penjualan), id_penjualan', 'DESC')
			->get()->row_array();
			
			$pch_id = explode('-', $transaksi['id_penjualan']);
			if(!is_null($transaksi)){
				$id_transaksi = $pch_id[2];
				$data['id_penjualan'] = 'T-'.date('ymd').'-'.intval($id_transaksi+1);
			}else{
				$data['id_penjualan'] = 'T-'.date('ymd').'-'.intval(1);
			}
			...
			for ($i=0; $i < $x; $i++) { 
				...
				}elseif($data['mode_jual'] == 'Harga Go Food'){
					$harga_jual = $barang['harga_jual_go_food'];
				}elseif($data['mode_jual'] == 'Harga Grab Food'){
					$harga_jual = $barang['harga_jual_grab_food'];
				}elseif($data['mode_jual'] == 'Harga Shopee Food'){
					$harga_jual = $barang['harga_jual_shopee_food'];
				} else {
				...
				$sisa_stok = $get_stok['stok'] - $data['jumlah'][$i];
				...
				$data_penjualan_detail	= [
					...
					'sisa_stok'			=> $sisa_stok,
					'ket_diskon'		=> $barang['diskon'] != 0 ? 'Diskon '.$barang['diskon'].'%' : '-',
				];
	...
	public function tambah_detail_barang_keluar($id_penjualan)
	{
		...
		if (!$this->form_validation->run()) {
			...
			if($trans['keterangan'] == 'Harga Reseller Cash'){
				...
				$harga_jual = $barang['harga_jual_reseller_tempo'];
			}elseif($trans['keterangan'] == 'Harga Go Food'){
				$harga_jual = $barang['harga_jual_go_food'];
			}elseif($trans['keterangan'] == 'Harga Grab Food'){
				$harga_jual = $barang['harga_jual_grab_food'];
			}elseif($trans['keterangan'] == 'Harga Shopee Food'){
				$harga_jual = $barang['harga_jual_shopee_food'];
			} else {
				$harga_jual = $barang['harga_jual'];
			}
			...
			$get_stok = $this->M_barang->get_barang_by_id($data['id_barang']);
			$sisa_stok = $get_stok['stok'] - $data['jumlah'];
